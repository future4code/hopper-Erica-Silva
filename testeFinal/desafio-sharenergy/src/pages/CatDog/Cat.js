...
import { Button, Div, H1, Img, Input } from "./StyledCat"
...
const Cat = () => {
  ...
  const showCat = () => {
    ...
    if (statuses[imgCat]) {
      return <Img src={`https://http.cat/${imgCat}`}></Img>
    }
   }
  ...
  return (
    <Div>
      <H1>Cat</H1>
      <Input placeholder="Digite um status code" value={imgCat} onChange={((event) => setImgCat(event.target.value))}></Input>
      {showCat()}
      
      <Button onClick={() => { navigate("/") }}>voltar</Button>
      <Button onClick={() => { navigate("/dog") }}>Ir para página de cachorros</Button>
    </Div>
  )
}
